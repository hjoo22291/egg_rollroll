...
/* 	//페이지 링크 눌렀을때 해당페이지 보여주기 
	...
	//---------------------추가부분---------css적용000000000000000000000000
	$(".adminpagelink_member").on('click', function () {
		  $.ajax({
		    url: 'adminmemberpaging',
		    type: 'post',
		    ...
		    dataType: 'json',
		    success: function (response) {
		      var table = $("<table>").attr("border", "3");
		      var thead = $("<thead>").append("<tr><th>아이디</th><th>이메일</th><th>가입일</th><th>누적신고</th><th>회원삭제</th></tr>");
		      var tbody = $("<tbody>");

		      for (let i = 0; i < response.length; i++) {
		        var row = $("<tr>").append(
		          "<td>" + response[i].memberid + "</td><td>" + response[i].email + "</td><td>" +
		          response[i].regtime + "</td><td>" + response[i].reported + "</td><td><button class='deletemember' id='" + response[i].memberid + "'>회원삭제</button></td>"
		        );
		        tbody.append(row);
		      }

		      table.append(thead, tbody);
		      $("#adminpagememberlist").empty().append(table);
		    },
		    error: function (request, status, e) {
		      alert("코드=" + request.status + "\n" + "메시지=" + request.responseText + "\n" + "error=" + e);
		    }
